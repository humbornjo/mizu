...
// sure the clone() method works as expected.
func init() {
...
// deduplication. When enabled, concurrent identical requests will be
// coalesced into a single request to prevent duplicate work. Default
// is disabled (false).
...
// determine cache expiration time from requests. The function should
// return nil key and 0 duration for requests that should not be
// cached. Default function returns nil key and 0 duration (no caching).
...
// expiration times. This helps prevent cache stampedes by spreading
// out expiration times. The function receives the original expiry
// duration and should return the adjusted duration. Default function
// reduces expiry by up to 10%.
...
// WithCleanupArbiter sets a custom function to determine when cache
// cleanup should occur. The function receives the response and should
// return true to trigger cleanup of expired entries. Default function
// triggers cleanup randomly with 0.1% probability (1 in 1000 requests).
...
// New creates a new cache interceptor with the given options. The
// interceptor provides response caching for Connect RPC unary calls
// with support for single-flight deduplication, custom key generation,
// jittered expiration, and automatic cleanup of expired entries.
